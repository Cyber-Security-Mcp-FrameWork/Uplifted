...
async def exception_handler(request: Request, exc: Exception):
    # 提取最后一帧的文件路径和行号
    # 如果路径包含 "Uplifted/src/"，只保留其后部分
    # 记录错误并返回 500 JSONResponse，body 中包含出错位置和消息
...
# 这里都是处理错误
# 当某个函数（路由处理函数或其他内部调用）抛异常时，捕获并打印，然后返回统一格式的错误响应，不让异常“冒泡”到全局。
def handle_server_errors(func):
    ...
    async def async_wrapper(*args, **kwargs):
        try:
            ...
            # 打印完整 traceback
            # 返回一个包含 status_code=500 和 detail 的标准字典
...
# timeout 超时装饰器
# 给任意（同步或异步）函数添加超时保护，如果执行超过指定秒数，就返回 408 Request Timeout。
def timeout(seconds: float):
    def decorator(func):
        #用 asyncio.wait_for 等待指定秒数，超时抛 HTTPException(408)
        @wraps(func)
        async def async_wrapper(*args, **kwargs):
            try:
        ...
        #启动后台线程执行 func，thread.join(seconds) 设超时，超时抛 HTTPException(408)
        @wraps(func)
        def sync_wrapper(*args, **kwargs):
